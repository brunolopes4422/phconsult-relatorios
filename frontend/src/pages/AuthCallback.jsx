...
export default function AuthCallback() {
  ...
  useEffect(() => {
    ...
    const state = params.get('state') // client_id
    ...
    api.get(`/auth/callback?code=${code}&state=${state}`)
      .then(() => {
        setStatus('success')
        // If opened in popup, close and reload parent
        if (window.opener) {
      ...
      <div className="card w-full max-w-sm p-8 text-center">
        ...
        {status === 'success' && (
          <>
            ...
            <p className="text-slate-500 mt-2">Conta Azul conectada com sucesso. Esta janela vai fechar automaticamente.</p>
          </>
        )}
        {status === 'error' && (
          <>
            ...
            <button className="btn-secondary mt-4 w-full" onClick={() => window.close()}>Fechar</button>
          </>
        )}
      </div>
    </div>
