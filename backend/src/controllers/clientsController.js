...
async function list(req, res) {
  const { data, error } = await supabase
    .from('clients')
    .select('id, name, status, ca_connected, ca_token_expires_at, documento, integration_type, omie_app_key, report_model')
    .order('name')
...
async function update(req, res) {
  const { name, status, integration_type, omie_app_key, omie_app_secret, report_model } = req.body
  ...
  if (report_model !== undefined) updates.report_model = report_model
